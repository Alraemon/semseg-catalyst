...
from util.loss_function import MultiChComboLoss
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    is_med = args.get('is_med', None)
    print(is_med)
    if is_med:
        criterion = MultiChComboLoss(ch_wts = tuple([0.4 if a==0 else 1 for a in range(args.classes)]))
    else:
    ...
    for module in modules_new:
        params_list.append(dict(params=module.parameters(), lr=args.base_lr* 10))# 新模块学习率是老模块学习率是10倍 #* 10
    ...
    train_data = dataset.SemData(split='train', data_root=args.data_root, data_list=args.train_list, transform=train_transform,
                                label_mapping= args.get('label_mapping', None), is_med= args.get('is_med', None))
    if args.distributed:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss_train, mIoU_train, mAcc_train, allAcc_train, mDice_train = train(train_loader, model, optimizer, epoch)
        if main_process():
            writer.add_scalar('loss_train', loss_train, epoch_log)
            writer.add_scalar('mIoU_train', mIoU_train, epoch_log)
            writer.add_scalar('mAcc_train', mAcc_train, epoch_log)
            writer.add_scalar('allAcc_train', allAcc_train, epoch_log)
            writer.add_scalar('mDice_train', mDice_train, epoch_log)
        ...
        if args.evaluate:
            loss_val, mIoU_val, mAcc_val, allAcc_val, mDice_val = validate(val_loader, model, criterion)
            if main_process():
                writer.add_scalar('loss_val', loss_val, epoch_log)
                writer.add_scalar('mIoU_val', mIoU_val, epoch_log)
                writer.add_scalar('mAcc_val', mAcc_val, epoch_log)
                writer.add_scalar('allAcc_val', allAcc_val, epoch_log)
                writer.add_scalar('mDice_val', mDice_val, epoch_log)
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        start_ch = 1 if args.get('is_med', None) else 0
        dice = sum(intersection_meter.val[start_ch:])*2 /(
               sum(intersection_meter.val[start_ch:]) + sum(union_meter.val[start_ch:]) + 1e-10)
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Epoch: [{}/{}][{}/{}] '
                        ...
                        'Dice {dice:0.4f}'.format(epoch+1, args.epochs, i + 1, len(train_loader),
        ...
        if main_process():
            ...
            writer.add_scalar('dice_train_batch', dice, current_iter)

    iou_class = intersection_meter.sum / (union_meter.sum + 1e-10) # 针对每个类，聚合所有sample的intersect和union，epoch末尾给出一个总的值。
    accuracy_class = intersection_meter.sum / (target_meter.sum + 1e-10)
    dice_class = intersection_meter.sum *2  / (union_meter.sum + intersection_meter.sum + 1e-10)
    mIoU = np.mean(iou_class)
    mAcc = np.mean(accuracy_class)
    mDice = np.mean(dice_class[start_ch:])
    allAcc = sum(intersection_meter.sum) / (sum(target_meter.sum) + 1e-10)
    if main_process():
        logger.info('Train result at epoch [{}/{}]: mIoU/mAcc/allAcc/mDice {:.4f}/{:.4f}/{:.4f}/{:.4f}.'.format(
                                                    epoch+1, args.epochs, mIoU, mAcc, allAcc, mDice))
    return main_loss_meter.avg, mIoU, mAcc, allAcc, mDice
...
def validate(val_loader, model, criterion):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        if ((i + 1) % args.print_freq == 0) and main_process():
            logger.info('Test: [{}/{}] '
                        ...
                        'Accuracy {accuracy:.4f}.'
                        'Dice {dice: .4f}'.format(i + 1, len(val_loader),
                                                          data_time=data_time,
                                                          batch_time=batch_time,
                                                          loss_meter=loss_meter,
                                                          accuracy=accuracy, 
    ...
    mDice = np.mean(dice_class[start_ch:])
    if main_process():
        logger.info('Val result: mIoU/mAcc/allAcc/mDice {:.4f}/{:.4f}/{:.4f}/{:.4f}.'.format(mIoU, mAcc, allAcc, mDice))
        for i in range(args.classes):
            logger.info('Class_{} Result: iou/accuracy/dice {:.4f}/{:.4f}{:.4f}.'.format(i, iou_class[i], accuracy_class[i], dice_class[i]))
        logger.info('<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<')
    return loss_meter.avg, mIoU, mAcc, allAcc, mDice
